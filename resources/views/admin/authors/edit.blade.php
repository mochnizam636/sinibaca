...
@section('content')
    <div class="max-w-xl mx-auto">
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-white">Edit Author</h2>
                <p class="text-sm text-zinc-400">Update author information.</p>
            </div>
        </div>

        <!-- Form Card -->
        <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden shadow-sm">
            <form action="{{ route('admin.authors.update', $author) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="bio" id="bio" rows="5"
                        class="w-full bg-zinc-950 border border-zinc-700 text-white rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder:text-zinc-600 outline-none resize-none">{{ old('bio', $author->bio) }}</textarea>
                    @error('bio')
                        <p class="text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
